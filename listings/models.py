...
class Listing(models.Model):
    ...
    rooms = models.PositiveIntegerField(
        null=True, blank=True, help_text="Number of rooms"
    )
    ...
    is_active = models.BooleanField(
        default=True, db_index=True, help_text="Visibility status of the listing"
    )
        ...
        if self.daily_enabled:
            if not self.price_per_day or self.price_per_day <= 0:
                raise ValidationError(
                    "If daily rental is enabled, please specify a positive price per day."
                )
        else:
